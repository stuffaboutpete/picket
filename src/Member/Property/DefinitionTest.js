describe('Member.Property.Definition', function(){
	...
	it('can be instantiated with a signature', function(){
		var property = new ClassyJS.Member.Property.Definition('public myProperty (string)');
		expect(property instanceof ClassyJS.Member.Property.Definition).toBe(true);
	});
	
	it('throws error if instantiated with no signature', function(){
		var expectedFatal = new ClassyJS.Member.Property.Definition.Fatal(
			'NON_STRING_SIGNATURE',
			'Provided type: undefined'
		);
		expect(function(){ new ClassyJS.Member.Property.Definition(); }).toThrow(expectedFatal);
	});
	...
	it('throws error if signature is not recognised', function(){
		...
		expect(function(){
			new ClassyJS.Member.Property.Definition('gibberish');
