describe('Member.Constant.Definition', function(){
	...
	it('can be instantiated with a signature', function(){
		var constantDefinition = new ClassyJS.Member.Constant.Definition(
			'public constant MY_CONSTANT'
		);
		expect(constantDefinition instanceof ClassyJS.Member.Constant.Definition).toBe(true);
	});
	
	it('throws error if instantiated with no signature', function(){
		var expectedFatal = new ClassyJS.Member.Constant.Definition.Fatal(
			'NON_STRING_SIGNATURE',
			'Provided type: undefined'
		);
		expect(function(){ new ClassyJS.Member.Constant.Definition(); }).toThrow(expectedFatal);
	});
	...
	it('throws error if signature is not recognised', function(){
		...
		expect(function(){
			new ClassyJS.Member.Constant.Definition('gibberish');
	...
	it('only recognises capitalized names', function(){
		...
		expect(function(){
			new ClassyJS.Member.Constant.Definition('public constant myConstant');
		}).toThrow(expectedFatal);
	});
