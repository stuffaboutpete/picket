describe('Type.Interface.Definition', function(){
	...
	it('can be instantiated with a signature', function(){
		var interfaceDefinition = new ClassyJS.Type.Interface.Definition('interface IMyInterface');
		expect(interfaceDefinition instanceof ClassyJS.Type.Interface.Definition).toBe(true);
	});
	
	it('throws error if instantiated with no signature', function(){
		var expectedFatal = new ClassyJS.Type.Interface.Definition.Fatal(
			'NON_STRING_SIGNATURE',
			'Provided type: undefined'
		);
		expect(function(){ new ClassyJS.Type.Interface.Definition(); }).toThrow(expectedFatal);
	});
	...
	it('throws error if instantiated with non string signature', function(){
		...
		expect(function(){ new ClassyJS.Type.Interface.Definition(123); }).toThrow(expectedFatal);
	});
	...
	it('throws error if signature does not contain \'interface\' keyword', function(){
		...
		expect(function(){
			new ClassyJS.Type.Interface.Definition('IMyInterface');
	...
	it('throws error if signature is not recognised', function(){
		...
		expect(function(){
			new ClassyJS.Type.Interface.Definition('Something interface Something');
		}).toThrow(expectedFatal);
	});
	...
	it('can parse signature with irregular whitespace', function(){
		var interfaceDefinition = new ClassyJS.Type.Interface.Definition(
